...
const VimEditor: React.FC<VimEditorProps> = ({ filename, content, onExit }) => {
  ...
  return (
    <div
      className="flex-grow flex flex-col h-full bg-terminal-bg text-terminal-fg font-mono text-sm"
    ...
    >
      ...
      <div className="flex justify-between items-center bg-gray-700 text-white px-2 h-6 font-bold text-xs">
        ...
        <span>
          {lines.length}L, {content.length}C
        </span>
      </div>
      <form
        onSubmit={handleCommandSubmit}
        className="flex items-center bg-terminal-bg h-6 px-2"
      >
        <input
          ref={inputRef}
          type="text"
