      ...
      {isGameRunning ? (
        ...
        <>
          ...
          <div className="flex-grow">
            {lines.map(line => (
              <div key={line.id} className={`whitespace-pre-wrap ${line.type === 'input' ? 'text-terminal-green' : ''}`}>
                {typeof line.text === 'string' ? line.text.split('\n').map((part, i) => <div key={i}>{part}</div>) : line.text}
              </div>
            ))}
          </div>
          {suggestions.length > 0 && (
              <div className="text-terminal-blue mt-1 mb-1 flex flex-wrap gap-x-4 gap-y-1">
                  {suggestions.map((s, i) => <span key={i}>{s}</span>)}
              </div>
          )}
          <form onSubmit={handleSubmit} className="flex items-center">
            <span className="text-terminal-yellow mr-2">{isAwaitingPassword ? 'Password:' : PROMPT}</span>
            <input
              ref={inputRef}
